...
class Network():
    ...
    def AddLayer(self, layer):
        ...
        if self.n_layers>=2:
            m = self.lyr[-1].N
            n = self.lyr[-2].N
            w = np.random.normal(size=[n,m])/np.sqrt(n)*4.
            self.W.append(w)
            w = np.random.normal(size=[m,n])/np.sqrt(n)*4.
            self.W_feedback.append(w)
            
            # temp = []
            # for i in range(self.n_layers-2):
            #     n = self.lyr[-1].N
            #     m = self.lyr[i+1].N
            #     w_BDFA = np.random.normal(size=[n,m])/np.sqrt(n)*4.
            #     temp.append(w_BDFA)
            # self.W_BDFA = temp

    def TopGradient(self, targets, mode=''):
        '''
        ...
        '''
        if (mode == 'BFA'):
            return MSE_p(self.lyr[0].h, targets) * self.lyr[0].sigma_p(self.lyr[0].h)
